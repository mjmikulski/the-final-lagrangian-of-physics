...
| 014 | [Linear-in-F terms with spectral coefficients](reports/014-linear-F-terms/) | The one unscanned order: terms linear in F (quadratic in ∂M). Twelve pointwise generators (6 even + 6 odd, exact over ℚ), all dynamical once the coefficients depend on the spectral frame (the constant φ, χ stay null); no ω² part, so nothing for the clock; inert on the canonical ansatz (006's Newton no-go stands); no vacuum condensate (the O(t²) part is a total derivative, the linear integral scales as t⁴); on the electron a sign-weighted reweighting of the static density with an unchanged core — and with exact spectral coefficients the relaxation runs onto the eigenvalue-crossing walls where those coefficients are singular |
