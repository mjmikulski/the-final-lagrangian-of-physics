...
| 014 | [Linear-in-F terms with spectral coefficients](reports/014-linear-F-terms/) | The one unscanned order: terms linear in F (quadratic in ∂M). Twelve pointwise generators (6 even + 6 odd; exact upper and lower bounds), all dynamical once the coefficients depend on the spectral frame (the constant φ, χ stay null); no ω² part, so no drive or brake in the fixed-velocity Legendre energy (a gyroscopic term remains for dynamics, unstudied); even sector inert on the canonical ansatz and the odd sector vanishing on its parity-symmetric configurations, so 006's Newton no-go stands there; for each scanned non-null class the uniform vacuum is a saddle at cubic order (null combinations such as φ have none; three sampled trial estimates far below the resolution of a relaxation, no global bound); on the electron a sign-weighted reweighting of the static density with the core structure kept — and with exact spectral coefficients the relaxation mostly runs to the eigenvalue-1/small-pair collisions where those coefficients are non-smooth |
