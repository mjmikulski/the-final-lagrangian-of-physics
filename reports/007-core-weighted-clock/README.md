...
1. **Blindness theorem (§1, exact):** any coefficient built pointwise
   from algebraic invariants of $M$ (traces of powers of $M\eta$, hence
   the potential and eigenvalues) is *identically constant* along
   boost-dressed configurations $M = o(x)M_0o(x)^\top$ — the
   $\eta$-similarity $M'\eta = o(M\eta)o^{-1}$ — and any *smooth*
   topological density scales as the cube of the dressing amplitude. So
   $c(M)$ and $c(\text{topology})$ reduce to constants on the canonical
   Newton ansatz and fall under report 006's no-go verbatim: **this
   mechanism is for the clock, not for Newton.**
...
3. **Kinetic windows (§3, two routes):** the velocity-space quadratic
   form vanishes identically in vacuum for every $c$ (safe), is PSD on
   the hedgehog at $c=0$ under the working $G$ metric (consistent with
   004), and opens exactly one negative — clock — direction at
   $c_{\rm clock} \approx -0.53..-0.65$ across core cells.
4. **Ladder series (§4, the decisive measurement):** with the local
   ...
   produces a clean interior well with
   $\boldsymbol{\omega_* = 0.8}$ (exactly the calibration target) and
   the boost density localized on $\sim100$ core sites. A by-product
   witness: transfer-ladder relaxations at large $\omega$ reach statics
   *below* report 004's polished minimum (4.8250 < 4.8347), so that
   minimum was local — consistent with the two-family structure reported
   by substrate-framework's P243.
...
- **Topological densities:** the normalized boost direction of the
  ...
  and is not an admissible smooth functional; every smooth density
  $q \sim \varepsilon^{ijk}u\cdot(\partial_iu\times\partial_ju)$ scales
  as $m^3$ (measured ratios 8.13, 8.03 under amplitude doubling — cubic).
  Cores only, again.
...
Consequence: on report 006's ansatz any $c(M)$ or smooth topological
coefficient is the constant $c(\text{vacuum})$, and the quadratic no-go
applies unchanged. The mechanism's entire value is for the clock.
...
| L4 | intensive, fresh-start | frozen mask | **interior well: $E(0.5)>E(0.8)<E(1.1)$, $\omega_*=0.8$, PR $\approx100$ sites** |
...
as a stand-alone witness (report 004's polished point was a local
minimum), consistent with the stable/unstable two-family structure that
...
- The weight is **frozen** in the successful run; the dynamic weight is
  exploitable (L1). A self-consistent dynamic weight needs a form whose
  support the field cannot cheaply extend — not constructed here.
...
| ladders L1–L4 | `ladder_series.py` |
