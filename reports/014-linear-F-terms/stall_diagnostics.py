...
crossing, Riesz). Usage: python stall_diagnostics.py B  (reads the committed original float64 endpoint fields in results/fields/)
"""
import json, os, sys
...
for tag, r in res.items():
    ...
    if not os.path.exists(fn):
        raise SystemExit(f'endpoint field missing: {fn} (committed original endpoints live in results/fields/)')
