...
confirms the virial ratio and, on **every clean row** (all separations of
all clean profiles, hence in particular the ceiling-setting and
minimal-$X$ rows): $E^{\rm int}_k>0$, $t_1>4/3$, $X>0$, each within 5% of
route 1; it persists its own clean ceiling and asserts it below its own
chain-7 witness ratio (within 1% of route 1), closing the $\alpha<0$
branch entirely within route 2; plus the cutoff-free gaussian $X>0$;
...
- 3×3 hedgehog field and lattice conventions: report 004 (merged); the
  ...
  in `results/lattice_cost_external.json` measured on the 004-line field
  (working repo `covariant_split` @ `0bcea47`).
