...
(review round 1) — and since the configurations below miss it by two to three
...
3. **The lattice agrees: fitted inertia prefactors within 2%, cost 10–30% low.** On the
   ...
   |---|---|---|---|
   | cost | 22–23 | 23–29 | 32 |
   | inertia | 64–66 | 63–67 | 64 |

   (ranges over the three zone thicknesses; the fitted prefactors are 65.5 and 65.4 against 64.4; `halo_scaling.py`, figure below). The cost deficit is largest
...
4. **Consequence: the fixed-J infimum is the static energy, so no
   ...
   that are stationary but not minima are not excluded by it, and nothing
...
5. **What fixed-J minimization in a box finds is a localized
   deformation.** Minimizing H_J in box 12 from a symmetry-broken seed
   at J = 1, 4, 10, 18 gives approximate configurations (residual
   gradients 3·10⁻³–3.5·10⁻², see the limitations) whose energy above
   E_static = 27.29 is 0.03 at J = 1 and 0.25 at J = 18, growing sublinearly and split about 40/60 between
   ...
   the static lattice hedgehog (`rotor_branch.py`, figure below).
   E/(2JΩ) lies between about 500 (J = 1) and 70 (J = 18): the clock
   diagnostic stays two to three orders of magnitude from equality at
   all four sampled points. The halo tilt of result 4 would cost
   J/R ≈ 0.2–3 in this box, far above the total excess of these
   configurations; equating J/R with that excess gives R ≈ 35–75, an
   order-of-magnitude estimate of the box size beyond which the
   extended tilt is the cheaper way to carry J. The infinite-volume
   infimum is out of this lattice's reach, and the box configurations
   are what fixed-J minimization finds from this seed in this box, not
   a measurement of it.
...
- The finite-box configurations are not converged along the soft
  box-adapted translation mode (final gradient norms 3·10⁻³–3.5·10⁻²),
  so they are neither certified stationary points nor certified
  minima and their energies are upper bounds; a single box (12) and a
  single seed. The crossover radius of result 5 is a heuristic
  comparison of two energy estimates, not an established crossover of
  branches with box size. A box-18 continuation exists in the development repository
